...
columnas = []
for i in range(len(tea.columns)):
    columnas.append(str(tea.columns[i]) + ' ' + str(round(tea.iloc[-1,i]*100,2)) + '%')

tea.columns = columnas
...
plt.xticks(size=50)
plt.yticks(size=60)
plt.savefig('fxtea.png',bbox_inches='tight')
...
writer = pd.ExcelWriter(f'ArgentinaForex.xlsx',engine='xlsxwriter')
table.to_excel(writer,f'Dollar Futures {pasado}')
